...
describe("state", () => {
  ...
  test("first run: zero flow and change count, totals computed", () => {
    ...
    expect(u.next.wallets["0xa"]).toEqual({ raw: { BNB: E18.toString(), USDT: "0", USDC: "0" }, usd: 500, firstSeen: "2026-09-04", lastChanged: "2026-09-04" });
  });
  ...
  test("second run: detects moves, signed net flow, keeps lastChanged for idle wallets", () => {
    ...
    expect(d2.changedWallets).toBe(2);
    expect(d2.next.wallets["0xa"].lastChanged).toBe("2026-09-04");
